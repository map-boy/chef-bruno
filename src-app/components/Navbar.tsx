...
const Navbar = () => {
  ...
  const handleMouseLeave = () => {
    closeTimer.current = setTimeout(() => setContentOpen(false), 200);
  };
  ...
  const contentLinks = [
    ...
    { name: 'Online Classes', path: '/classes', badge: 'Live' },
  ];

  const handleLogout = async () => { await signOut(auth); };
  const isAdmin = user?.email && ADMIN_EMAILS.includes(user.email);
  const isContentActive = ['/videos', '/books', '/blog', '/classes'].includes(location.pathname);
        ...
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className={cn('text-xl font-serif font-bold tracking-tight transition-colors leading-none', isSolid ? 'text-stone-900' : 'text-white')}>
                CHEF BRUNO
              </span>
              <span className={cn('text-[8px] uppercase tracking-[0.4em] font-bold mt-1', isSolid ? 'text-stone-400' : 'text-stone-300')}>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span className={cn('absolute bottom-0 left-0 h-px bg-amber-600 transition-all duration-300',
              ...
              <AnimatePresence>
                {contentOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8 }}
                    className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-52 bg-white rounded-xl shadow-2xl border border-stone-100 overflow-hidden"
                  >
                    {contentLinks.map(cl => (
                      ...
                      >
                        ...
                        <span className={cn(
                          ...
                          cl.path === '/classes' ? 'text-emerald-700 bg-emerald-100' : 'text-stone-400 bg-stone-100'
                        )}>
                          {cl.badge}
                        </span>
                      </Link>
            ...
            {isAdmin && (
              <Link to="/admin/dashboard" className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isSolid ? 'text-stone-700' : 'text-stone-100')} title="Admin Panel">
                <User size={20} />
              </Link>
            )}

            {user ? (
              <button onClick={handleLogout} className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isSolid ? 'text-stone-700' : 'text-stone-100')} title="Logout">
                <LogOut size={20} />
              </button>
            ) : (
              <Link to="/admin/login" className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isSolid ? 'text-stone-700' : 'text-stone-100')} title="Admin Login">
                <User size={20} />
              </Link>
            )}
          </div>
          ...
          <div className="lg:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className={cn('p-2', isSolid ? 'text-stone-900' : 'text-white')}>
          ...
          >
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)}
                className={cn('text-xl font-medium flex justify-between items-center group py-2',
            ...
            <div className="border-t border-white/10 pt-4">
              ...
              {contentLinks.map(cl => (
                <Link key={cl.path} to={cl.path} onClick={() => setIsOpen(false)}
                  className={cn('flex items-center justify-between py-3 text-lg font-medium',
                ...
                >
                  ...
                  <span className={cn('text-[9px] px-2 py-0.5 rounded-full',
                    cl.path === '/classes' ? 'text-emerald-400 bg-emerald-900' : 'text-stone-500 bg-stone-800')}>
                    {cl.badge}
                  </span>
                </Link>
              ))}
            ...
            <div className="pt-4 flex justify-center space-x-8 border-t border-white/10">
              {isAdmin && (
                <Link to="/admin/dashboard" onClick={() => setIsOpen(false)} className="text-stone-400 hover:text-white">Admin Panel</Link>
              )}
              {user
