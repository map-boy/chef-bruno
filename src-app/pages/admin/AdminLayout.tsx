...
import {
  LayoutDashboard, Bed, Calendar, BookOpen, Utensils,
  Settings, HelpCircle, LogOut, Loader2, ChevronRight,
  Menu, X, Hotel, MessageSquare, Video, PenLine, BookMarked, Film
} from 'lucide-react';

const menuItems = [
  { name: 'Dashboard',       icon: <LayoutDashboard size={20} />, path: '/admin/dashboard' },
  { name: 'Reservations',    icon: <Hotel size={20} />,           path: '/admin/bookings' },
  ...
  { name: 'Online Classes',  icon: <Video size={20} />,           path: '/admin/classes' },
  { name: 'Blog',            icon: <PenLine size={20} />,         path: '/admin/blog' },
  { name: 'Books',           icon: <BookMarked size={20} />,      path: '/admin/books' },
  { name: 'Videos',          icon: <Film size={20} />,            path: '/admin/videos' },
...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex overflow-hidden">
      {/* Sidebar */}
      <aside className={`bg-stone-900 border-r border-stone-800 transition-all duration-300 z-50 fixed lg:static inset-y-0 left-0 ${
        ...
        <div className="h-full flex flex-col">
          <div className="p-6 border-b border-stone-800 flex items-center justify-between">
            ...
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="lg:hidden text-stone-400 hover:text-white">
              <X size={20} />
            </button>
          </div>
              ...
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSidebarOpen && (
                      <span className="text-sm font-bold uppercase tracking-widest grow">{item.name}</span>
                    )}
