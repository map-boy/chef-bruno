        ...
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className={cn('text-xl font-serif font-bold tracking-tight transition-colors leading-none', isScrolled ? 'text-stone-900' : 'text-white')}>
                CHEF BRUNO
              </span>
              <span className={cn('text-[8px] uppercase tracking-[0.4em] font-bold mt-1', isScrolled ? 'text-stone-400' : 'text-stone-300')}>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span className={cn('absolute bottom-0 left-0 h-px bg-amber-600 transition-all duration-300', location.pathname === link.path ? 'w-full' : 'w-0 group-hover:w-full')} />
              </Link>
            ))}
            ...
            <div className="relative" onMouseLeave={() => setContentOpen(false)}>
              <button
                ...
                className={cn(
                  ...
                  isContentActive ? 'text-amber-600' : (isScrolled ? 'text-stone-600 hover:text-stone-900' : 'text-stone-200 hover:text-white')
                )}
              >
                Content
                <ChevronDown size={10} className={`transition-transform ${contentOpen ? 'rotate-180' : ''}`} />
                <span className={cn('absolute bottom-0 left-0 h-px bg-amber-600 transition-all duration-300', isContentActive ? 'w-full' : 'w-0 group-hover:w-full')} />
              </button>
              <AnimatePresence>
                {contentOpen && (
                  <motion.div
            ...
            <Link to="/book" className="bg-[#C9973A] hover:bg-[#b08432] text-white px-8 py-3 text-[10px] font-bold uppercase tracking-[0.2em] transition-all active:scale-95 shadow-md">
              Book Now
            </Link>

            {isAdmin && (
              <Link to="/admin/rooms" className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isScrolled ? 'text-stone-700' : 'text-stone-100')} title="Admin Panel">
                <User size={20} />
              </Link>
            )}

            {user ? (
              <button onClick={handleLogout} className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isScrolled ? 'text-stone-700' : 'text-stone-100')} title="Logout">
                <LogOut size={20} />
              </button>
            ) : (
              <Link to="/admin/login" className={cn('p-2 rounded-full hover:bg-black/10 transition-colors', isScrolled ? 'text-stone-700' : 'text-stone-100')} title="Admin Login">
                <User size={20} />
              </Link>
            )}
          </div>
          ...
          <div className="lg:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className={cn('p-2', isScrolled ? 'text-stone-900' : 'text-white')}>
          ...
          >
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)}
                className={cn('text-xl font-medium flex justify-between items-center group py-2', location.pathname === link.path ? 'text-amber-500' : 'text-stone-300 hover:text-white')}
              >
                {link.name}
            ...
            <div className="border-t border-white/10 pt-4">
              ...
              {contentLinks.map(cl => (
                <Link key={cl.path} to={cl.path} onClick={() => setIsOpen(false)}
                  className={cn('flex items-center justify-between py-3 text-lg font-medium', location.pathname === cl.path ? 'text-amber-500' : 'text-stone-300 hover:text-white')}
                >
                  {cl.name}
            ...
            <Link to="/book" onClick={() => setIsOpen(false)} className="bg-amber-600 text-white px-6 py-4 text-lg font-bold text-center active:scale-95 transition-transform mt-2">
              Book Now
            </Link>

            <div className="pt-4 flex justify-center space-x-8 border-t border-white/10">
              {isAdmin && <Link to="/admin/rooms" onClick={() => setIsOpen(false)} className="text-stone-400 hover:text-white">Admin Panel</Link>}
              {user
